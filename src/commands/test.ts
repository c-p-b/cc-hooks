...
export class TestCommand {
  ...
  async execute(eventFile?: string, options: TestOptions = {}): Promise<void> {
    ...
    const files = eventFile 
      ? [this.resolveEventFile(eventFile)]
    ...
    try {
      ...
      try {
        ...
        if (exitCode === 2) {
          ...
          const hasBlockingHooks = matchingHooks.some(h => 
            h.outputFormat === 'text' && 
            h.exitCodeMap && 
        ...
        if (options.verbose && error.stderr) {
          ...
          console.log(chalk.gray(error.stderr.split('\n').map((l: string) => '    ' + l).join('\n')));
        }
      }
    } catch (error) {
  ...
  private getMatchingHooks(event: ClaudeHookEvent, config: any): any[] {
    ...
    return hooks.filter((hook: any) => {
      ...
      if ((event.hook_event_name === 'PreToolUse' || event.hook_event_name === 'PostToolUse') && hook.matcher) {
