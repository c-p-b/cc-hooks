...
describe('LogCleaner', () => {
  ...
  describe('cleanupIfNeeded', () => {
    it('should create logs directory if it does not exist', async () => {
      ...
      expect(mkdirMock).toHaveBeenCalledWith(mockLogsDir, { recursive: true });
    ...
    it('should remove stale lock and acquire new one', async () => {
      ...
      expect(unlinkMock).toHaveBeenCalledWith(path.join(mockLogsDir, '.cleanup.lock'));
  ...
  describe('performCleanup', () => {
    it('should delete files older than 7 days', async () => {
      ...
      readdirMock.mockResolvedValue(['session-old.jsonl', 'session-recent.jsonl', 'not-a-log.txt']);
      ...
      expect(unlinkMock).toHaveBeenCalledWith(path.join(mockSessionsDir, 'session-old.jsonl'));
    ...
    it('should delete files when total size exceeds 500MB', async () => {
      ...
      readdirMock.mockResolvedValue(['session-1.jsonl', 'session-2.jsonl', 'session-3.jsonl']);
      ...
      expect(unlinkMock).toHaveBeenCalledWith(path.join(mockSessionsDir, 'session-1.jsonl'));
