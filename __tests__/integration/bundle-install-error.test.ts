...
describe('Bundle Installation Error Handling', () => {
  ...
  it('should fail entire bundle installation if any hook is invalid', async () => {
    ...
    await expect(installCommand.execute('bad-bundle')).rejects.toThrow(
      /Failed to load hook from invalid.json/,
    );
  ...
  it('should fail with clear error for malformed JSON in bundle', async () => {
    ...
    await expect(installCommand.execute('json-error')).rejects.toThrow(
      /Failed to load hook from broken.json/,
    );
  });
});
